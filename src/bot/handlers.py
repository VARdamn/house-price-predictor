...
from src.bot.keyboards import yes_no_park
...
info_text = 'С помощью бота вы можете узнать ожидаемую рыночную стоимость аренды жилья в Москве.\nИспользуйте /predict для прогноза цены на аренду жилья. По запросу бота поочередно вводите характеристики квартиры, которые у вас есть.\nПользуйтесь подсказками бота, если не знаете, что выбрать, или характеристика для вас не важна. Такие данные будут заменены средними значениями.'
...
async def step3(message: Message, state: FSMContext):
    ...
    await message.answer("Есть ли мебель в квартире?", reply_markup=kb.yes_no_furn)

@router.message(InputForm.with_furniture)
@router.callback_query(F.data.startswith('ynf:'))
...
async def step7(message: Message, state: FSMContext):
    ...
    await state.set_state(InputForm.num_floors)
    await message.answer("Нужен ли паркинг?", reply_markup=yes_no_park)

@router.message(InputForm.parking)
@router.callback_query(F.data.startswith('ynp:'))
