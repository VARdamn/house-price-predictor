...
def format_stdout(record, level_padding=7):
    ...
    formatted_message = (
        f"<green>{time}</green> | <blue>{level}</blue> | <level>{message}</level>"
    )
...
def setup_logger():
    format_error = "<green>{time:HH:mm:ss.SS}</green> | <blue>{level}</blue> | <cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> | <level>{message}</level>" # noqa
    ...
    logger.add(
        ...
        colorize=False,
        format=lambda record: format_file(record, clean_brackets(format_error)),
        level='ERROR'
    )
